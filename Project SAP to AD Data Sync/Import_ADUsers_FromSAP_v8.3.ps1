...
foreach ($Record in $UpdatesRecords)
{
    ...
    #Execute set-aduser below only if $sam is in AD and also is in the excel file, else ignore#
		If($SAMinAD -ne $null -and $SAMinAD -ne '')
		{
            ...
            #Get-ADUser -LDAPFilter "(description=$EmployeeID)" | Set-ADUser -Replace @{Title=$Title;Department=$Department;} 
			$TimeStamp = Get-Date -Format yyyyMMddHHmm
			$TimeStamp + " : " + $EmployeeID + " modified to Title: [" + $Title + "] under Department: [" + $Department + "] reporting to Manager: " + $ManagerID  | Out-File $logfile -Append
