...
$LeftFile = Import-Csv -path "$FolderPath\$SAPDeltaFileName"
$RightFile = Import-Csv -path "$FolderPath\$DeptSectionsFileName"

Join-Object -Left $LeftFile -Right $RightFile -LeftJoinProperty Section -RightJoinProperty Section -Type AllInLeft -RightProperties Department | Export-Csv -NoTypeInformation "$FolderPath\MergedUpdates.csv"
...
$UpdatesRecords = Import-Csv -path $MergedUpdatesFile
...
foreach ($Record in $UpdatesRecords)
{
    ...
    #Execute set-aduser below only if $sam is in AD and also is in the excel file, else ignore#
		If($SAMinAD -ne $null -and $SAMinAD -ne '')
		{
			...
			$EmployeeID + " modified to Title: [" + $Title + "] under Department: [" + $Department + "] reporting to Manager: " + $ManagerID  | Out-File $logfile -Append
            $MailBodyContent += $EmployeeID + " modified to Title: [" + $Title + "] under Department: [" + $Department + "] reporting to Manager: " + $ManagerID + "<br/>"
