...
#Informs by email in absence of SAP delta file that no Sync operations were performed.
#Added names against the EmployeeID.
#--------------------------------------------------------------------------------------------------------
#Changes in Version 8.6
#Forced explicit recognization of parameters
#Renamed $logdate variable to $myTimeStamp
#New feature, modify the MergedUpdates file name with timestamp to avoid repeated processing the next time this script is run.
#--------------------------------------------------------------------------------------------------------
...
(
    ...
    [string]$SAPDeltaFileName,

    [Parameter(Mandatory=$True,Position=3)]
    [string]$DeptSectionsFileName,
...
To =  $MailTo
From = $MailFrom
Subject = "SAP to AD Employee Sync Report " + $myTimeStamp
...
#Check if $SAPDeltaFileName is there
If(-NOT (Test-Path $FolderPath\$SAPDeltaFileName))
{
    Send-MailMessage @smtpsettings -Body "$FolderPath\$SAPDeltaFileName NOT found...<br/> NO Sync Action Performed...<br/> Exiting Script..." -BodyAsHtml -Encoding ([System.Text.Encoding]::UTF8)
    exit
}
...
Join-Object -Left $LeftFile -Right $RightFile -LeftJoinProperty Section -RightJoinProperty Section -Type AllInLeft -RightProperties Department | Export-Csv -NoTypeInformation "$FolderPath\MergedUpdates $myTimeStamp.csv" -Delimiter '~'

$MergedUpdatesFile = $FolderPath + "\MergedUpdates $myTimeStamp.csv"
Rename-Item "$FolderPath\$SAPDeltaFileName" -NewName "$FolderPath\$SAPDeltaFileName - Processed $myTimeStamp.csv"

$logfile = $FolderPath + "\" + "AD SAP Data Sync Report $myTimeStamp.log"
$MailBodyContent = "SAP to AD Employee Sync Report... <br/><br/><br/>"
...
foreach ($Record in $UpdatesRecords)
{
    ...
    #Execute set-aduser below only if $sam is in AD and also is in the excel file, else ignore#
		...
		{
            #get name of the AD object User
            $Name = Get-ADUser -LDAPFilter "(description=$EmployeeID)" | Select Name
			...
			$TimeStamp + " : " + $EmployeeID + "-" + $Name + " modified to Title: [" + $Title + "] under Department: [" + $Department + "], Section: [" + $Section + "] reporting to Manager: [" + $ManagerID + "]"  | Out-File $logfile -Append
            $MailBodyContent += $EmployeeID + "-" + $Name + " modified to Title: [" + $Title + "] under Department: [" + $Department + "], Section: [" + $Section + "] reporting to Manager: [" + $ManagerID + "]<br/>"
		...
		{ #Log error for users that are not in Active Directory or with no Logon name in excel file
			$TimeStamp = Get-Date -Format yyyyMMddHHmm
            $TimeStamp + " : " + $EmployeeID + "-" + $Name + " not modified because it does not exist in AD" | Out-File $logfile -Append
			$MailBodyContent += $EmployeeID + "-" + $Name + " not modified because it does not exist in AD <br/>"
		}
}
