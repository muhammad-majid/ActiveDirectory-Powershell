...
Function Create-Users
{
  ...
  Import-CSV $newpath | ForEach-Object {
	...
	(insertTimeStamp) + "Running iteration $i.." | Out-File $log -append
	...
	If (($_.GivenName -eq "") -Or ($_.LastName -eq ""))
	{
		...
		If($exists)
		{
			...
			(insertTimeStamp) + "[SKIP]`t User $($sam) ($($_.GivenName) $($_.LastName)) already exists or returned an error!" | Out-File $log -append
			$i++
			return
		}
          ...
          Try
          {
            ...
            (insertTimeStamp) + "[INFO]`t Renamed $($sam) to $($_.GivenName) $($_.LastName)`r`n" | Out-File $log -append
			
			If (($_.CopyGroupMembershipsFrom -ne ""))
			{
			Write-Host "Copying group memberships from $($_.CopyGroupMembershipsFrom)`r`n"
            (insertTimeStamp) + "Copying group memberships from $($_.CopyGroupMembershipsFrom).." | Out-File $log -append
